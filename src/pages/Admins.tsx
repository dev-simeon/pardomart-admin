...
const TotalAdminsIcon = () => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    <ellipse cx="8.24711" cy="6.41667" rx="3.66667" ry="3.66667" stroke="#898A8D" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" />
    ...
    <path d="M14.6696 2.86816C16.292 3.28357 17.4267 4.74549 17.4267 6.42025C17.4267 8.09501 16.292 9.55693 14.6696 9.97233" stroke="#898A8D" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" />
...
export default function Admins() {
  ...
  const { data: totalAdmins, isLoading: loadingAdmins } = useUsersCount(Role.Admin);
  const totalAdminsDisplay = loadingAdmins ? "..." : (totalAdmins ?? 0).toLocaleString();
  ...
  const columns: ColumnDef<User>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        const initials = (u.name || u.email || "?")
          ...
          .toUpperCase();
        return (
          <Link to={`/admins/${u.id ?? ""}`} className="flex items-center gap-1.5 hover:opacity-80 transition-opacity">
    ...
    { accessorKey: "role", header: "Role", meta: { headerClassName: "min-w-[120px]" } },
    {
      header: "Status",
      cell: ({ row }) => (
    ...
    { header: "Joined", cell: ({ row }) => formatDate(row.original.createdAt), meta: { headerClassName: "min-w-[140px]" } },
    {
      header: "Action",
      cell: () => (
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <CustomerStatCard icon={<TotalAdminsIcon />} title="Total Admins" value={totalAdminsDisplay} />
        <CustomerStatCard icon={<ActiveAdminsIcon />} title="Active Admins" value="128" />
        <CustomerStatCard icon={<AdminsWithAccessIcon />} title="Full Access" value="89" />
        <CustomerStatCard icon={<PendingAdminsIcon />} title="Pending Approval" value="14" />
      </div>
      ...
      <div className="p-0">
        <div className="space-y-4">
          <DataTable
            ...
            loading={isLoading}
            enableRowSelection
            onRowClick={(row: Row<User>) => navigate(`/admins/${row.original.id}`)}
            toolbar={
              <DataTableToolbar
                tabs={[{ id: "all", label: "All Admins", count: loadingAdmins ? undefined : totalAdmins ?? 0 }]}
                activeTab={"all"}
                searchColumn={"name"}
                ...
                onExport={() => {}}
                onFilter={() => {}}
                ctaButton={{ label: "Add Admin", onClick: () => navigate("/") }}
              />
            }
          />
        </div>
