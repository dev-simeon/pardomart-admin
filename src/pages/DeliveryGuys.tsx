...
const TotalDeliveryGuysIcon = () => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    <ellipse cx="8.24723" cy="6.41667" rx="3.66667" ry="3.66667" stroke="#898A8D" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M2.75 19.25V17.4167C2.75 15.3916 4.39162 13.75 6.41667 13.75H10.0833C12.1084 13.75 13.75 15.3916 13.75 17.4167V19.25" stroke="#898A8D" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M14.6694 2.86816C16.2919 3.28357 17.4266 4.74549 17.4266 6.42025C17.4266 8.09501 16.2919 9.55693 14.6694 9.97233" stroke="#898A8D" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M19.25 19.2502V17.4169C19.2404 15.7527 18.1113 14.3037 16.5 13.8877" stroke="#898A8D" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

const NewDeliveryGuysIcon = () => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M15.8125 11V6.1875M15.8125 6.1875V5.5M15.8125 6.1875C15.8125 6.875 18.5625 9.625 18.5625 10.3125M18.5625 10.3125V5.5M18.5625 10.3125V11M3.4375 19.25V18.5625C3.4375 17.1038 4.01696 15.7049 5.04841 14.6734C6.07986 13.642 7.47881 13.0625 8.9375 13.0625H10.3125C11.7712 13.0625 13.1701 13.642 14.2016 14.6734C15.233 15.7049 15.8125 17.1038 15.8125 18.5625V19.25M13.0625 6.875C13.0625 7.78668 12.7003 8.66102 12.0557 9.30568C11.411 9.95034 10.5367 10.3125 9.625 10.3125C8.71332 10.3125 7.83898 9.95034 7.19432 9.30568C6.54966 8.66102 6.1875 7.78668 6.1875 6.875Z" stroke="#6A717F" strokeWidth="2" strokeLinejoin="round" />
  </svg>
);

const TotalDeliveryIcon = () => (
  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M15.5833 3.66699H6.41659C5.40406 3.66699 4.58325 4.4878 4.58325 5.50033V17.417C4.58325 18.4295 5.40406 19.2503 6.41659 19.2503H15.5833C16.5958 19.2503 17.4166 18.4295 17.4166 17.417V5.50033C17.4166 4.4878 16.5958 3.66699 15.5833 3.66699Z" stroke="#6A717F" strokeWidth="2" />
    <path d="M8.25 8.25H13.75M8.25 11.9167H13.75M8.25 15.5833H11.9167" stroke="#6A717F" strokeWidth="2" strokeLinecap="round" />
  </svg>
);
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-sans text-[15px] font-normal leading-normal text-[#131523]">{u.name || "Unknown"}</span>
          </div>
        );
      },
      meta: { headerClassName: "min-w-[220px]" },
    },
    { accessorKey: "email", header: "Email Address", meta: { headerClassName: "min-w-[180px]" } },
    {
      header: "Status",
      cell: ({ row }) => (
        row.original.active ? (
          <div className="inline-flex h-6 items-center justify-start rounded bg-[#C4F8E2] px-4">
            <span className="text-left font-sans text-sm font-normal leading-5 text-[#06A561]">Active</span>
          </div>
        ) : (
          <div className="inline-flex h-6 items-center justify-start rounded bg-[#E6E9F4] px-2.5">
            <span className="text-left font-sans text-sm font-normal leading-5 text-[#5A607F]">Inactive</span>
          </div>
        )
      ),
      meta: { headerClassName: "min-w-[120px]" },
    },
    { header: "Joined", cell: ({ row }) => (row.original.createdAt ? new Date(row.original.createdAt).toLocaleDateString() : ""), meta: { headerClassName: "min-w-[140px]" } },
    {
      header: "Action",
      cell: () => (
        <div className="flex items-center gap-2">
          <button className="transition-opacity hover:opacity-70"><MessageIcon /></button>
          <button className="transition-opacity hover:opacity-70"><DeleteIcon /></button>
        </div>
      ),
      meta: { headerClassName: "min-w-[120px]" },
    },
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <DeliveryGuysStatCard icon={<TotalDeliveryGuysIcon />} title="Total Delivery guys" value={totalDeliveryPersonsDisplay} />
        <DeliveryGuysStatCard icon={<NewDeliveryGuysIcon />} title="New Delivery guys" value="676" />
        <DeliveryGuysStatCard icon={<TotalDeliveryIcon />} title="Total Delivery" value="1,116" />
        <DeliveryGuysStatCard icon={<TotalReturnsIcon />} title="Total Returns" value="186" />
      </div>
      ...
      <div className="p-0">
        <div className="space-y-4">
          <DataTable
            ...
            toolbar={
              <DataTableToolbar
                tabs={[{ id: "all", label: "All Delivery guys", count: loadingDeliveryPersons ? undefined : totalDeliveryPersons ?? 0 }]}
                activeTab={"all"}
                searchColumn={"name"}
